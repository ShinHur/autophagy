...
| 파일 | 내용 | 권한 |
|---|---|---|
| `template.<format>` | 소유자가 최초 DM으로 준 원본 양식 | 0600 |
...
2. agent는 `register --name <재사용-이름> --template <수신파일>`을 **한 번** 실행한다.
   ...
   함께 보관한다.
3. 이후 소유자는 같은 양식을 다시 보내지 않는다. agent는 `templates-show`의 필드를 DM으로
   확인하고 `generate --template-name <이름> --fields-json …`으로 저장 원본을 반복
   사용한 뒤 생성본만 검토 DM한다.
4. **제출은 항상 소유자 본인이 한다** — 스킬은 문서를 외부 기관에 제출하지 않는다.
