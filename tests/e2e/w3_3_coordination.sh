...
set -euo pipefail
for key in E2E_REMOTE_HOST E2E_PRIMARY_ACCOUNT E2E_PEER_ACCOUNT E2E_APPROVAL_LOG E2E_DISCORD_GUILD_NAME; do
  [[ -n "${!key:-}" ]] || { echo "SKIP w3_3_coordination: missing $key (requires live gateways, Discord, and calendar services)"; exit 77; }
done
readonly HOST="$E2E_REMOTE_HOST"
readonly PRIMARY_ACCOUNT="$E2E_PRIMARY_ACCOUNT"
readonly PEER_ACCOUNT="$E2E_PEER_ACCOUNT"
readonly PEER_READY_TIMEOUT_S=90
repo="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"

agent() {
  local command="E2E_APPROVAL_LOG=$(printf '%q' "$E2E_APPROVAL_LOG") E2E_DISCORD_GUILD_NAME=$(printf '%q' "$E2E_DISCORD_GUILD_NAME") $1"
  ssh "$HOST" "sudo -n -u $PRIMARY_ACCOUNT -H bash -lc $(printf '%q' "$command")"
}
peer() { ssh "$HOST" "sudo -n -u $PEER_ACCOUNT -H bash -lc $(printf '%q' "$1")"; }
peer_unit() {
...
push() { ssh "$HOST" "sudo -n -u $PRIMARY_ACCOUNT -H bash -c 'umask 077; cat > \$HOME/$2'" < "$1"; }
...
for role_account in "primary:$PRIMARY_ACCOUNT" "peer:$PEER_ACCOUNT"; do
  role="${role_account%%:*}"
