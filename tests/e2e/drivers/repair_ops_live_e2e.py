#!/usr/bin/env python3
"""Exercise the W6-2 owner-gated repair chain in a temporary clone.

Precondition: this infrastructure-bound driver needs ``--source`` to name a
repair-runtime checkout with the ``git`` executable available; it exits 77 when
the source is unavailable so scenario runners can report a skip.
"""
    ...
    environment.update(
        {
            ...
            "AUTOPHAGY_OWNER_ID": "owner-primary",
...
def main(argv: list[str]) -> int:
    ...
    if not source.is_dir():
        print("SKIP repair-ops-live: source checkout is unavailable")
        return 77
    with tempfile.TemporaryDirectory(prefix="autophagy-w62-live-") as temporary:
        root = Path(temporary)
        _run_case(source, root, RunSpec("t_w62owner", "owner-primary", True, False, RepairPhase.COMPLETED, LifecycleState.DONE))
        ...
        _run_case(source, root, RunSpec("t_w62regress", "owner-primary", True, True, RepairPhase.REOPENED, LifecycleState.REOPENED))
