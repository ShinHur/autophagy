...
SKILL: Final = "managed-hello-scenario"
MESSAGE_ID: Final = "e2e-publish-message"
OWNER_ID: Final = "100000000000000012"
...
CHANNEL_ID: Final = "100000000000000013"
...
class World:
    ...
    def create(cls, root: Path) -> World:
        ...
        _ = world.git(seed, "config", "user.name", "publisher-primary")
        _ = world.git(seed, "config", "user.email", "publisher-primary@example.invalid")
        ...
        _ = world.git(world.checkout, "config", "user.name", "publisher-primary")
        _ = world.git(world.checkout, "config", "user.email", "publisher-primary@example.invalid")
        _ = world.source.mkdir(parents=True)
        _ = world.home.mkdir(mode=0o700)
        _ = world.allowed.write_text(
            f"publisher-primary@example.invalid {world.key('publisher').with_suffix('.pub').read_text(encoding='utf-8')}",
            encoding="utf-8",
        )
...
def line(output: str, prefix: str) -> str:
    item = next((candidate for candidate in output.splitlines() if candidate.startswith(prefix)), "")
    return re.sub(r"\b[a-f0-9]{64}\b", "<digest>", item)
...
def case_collision(root: Path) -> dict[str, JsonValue]:
    ...
    _ = world.require_sync()
    _ = world.live.mkdir()
    _ = (world.live / "hello-scenario").mkdir()
...
def case_prefix_forgery(root: Path) -> dict[str, JsonValue]:
    world = World.create(root)
    code = "from pathlib import Path; import sys; import automation.skill_store as store; store.STORE_ROOT=Path(sys.argv[1]); store._require_root=lambda:None; sys.argv=['skill_store.py','install','--skill','managed-hello-scenario','--hash','0'*64]; raise SystemExit(store.main())"
