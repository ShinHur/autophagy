"""W2-6 remote actuator — runs on the primary test account, observes only.
...
external effect is fixed-uuid5 synthetic vectors in the configured test collection, removed by
...
def meeting_env(work: Path) -> dict[str, str]:
    ...
    if not config.exists():
        config.write_text(
            json.dumps({"my_names": "owner-primary,소유자", "agent_id": "agent-primary"}),
            encoding="utf-8",
        )
    return {
