...
USER_AGENT = "scenario-bank-report-hub/1"
...
def agents_log_channel() -> str:
    guild_name = os.environ["E2E_DISCORD_GUILD_NAME"]
    guild = next(g for g in api("GET", "/users/@me/guilds") if g["name"] == guild_name)
    return next(
        c["id"]
