...
(stub site_mail_backend repo/state.db, stub GLM + non-GLM LLMs, stub site_mail_backend-send binary,
...
for the owner's manual reaction, accepted only inside this runner's E2E env).
...
OWNER = "100000000000000017"
PEER_BOT = "100000000000000018"
CHANNEL = "100000000000000019"
...
SEND_STUB = """#!/bin/sh
base="$(dirname "$0")"
{ printf '%s' "$*" | tr '\\n' ' '; printf '\\n'; } >> "$base/site_mail_backend-send-calls.log"
...
def _setup(work: Path, root: Path) -> tuple[dict[str, str], str]:
    repo = work / "repo"
    (repo / "site_mail_backend").mkdir(parents=True)
    (repo / "site_mail_backend" / "__init__.py").touch()
    (repo / "site_mail_backend" / "main.py").write_text("raise SystemExit(64)\n", encoding="utf-8")
    ...
    _stub(work / "site_mail_backend-send-stub", SEND_STUB)
    ...
    env = {
        ...
        "TRIAGE_MAILON_PYTHON": str(work / "site_mail_backend-send-stub"),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="w4-mail-bank-") as tmp:
        ...
        send_log = work / "site_mail_backend-send-calls.log"
        ...
        obs["approved_send"] = {
            ...
            "site_mail_backend_send_calls": _count(send_log),
            "send_argv_is_site_mail_backend_send": "site_mail_backend.main send" in (send_log.read_text(encoding="utf-8") if send_log.exists() else ""),
        ...
        bypass_call = ToolCall("python3", {"command": "python3 -m site_mail_backend.main send --to bypass@example.invalid --subject x --body y"})
