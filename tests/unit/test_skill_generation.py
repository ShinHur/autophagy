...
def test_router_when_auto_draft_routes_then_uses_only_w1_8_with_auto_source(tmp_path: Path) -> None:
    ...
    pipeline = FakePipeline(PipelineExit.AWAITING_OWNER)
    deploy_root = Path(synthetic_env()["AUTOPHAGY_DEPLOY_ROOT"])
    router = PipelineRouter(deploy_root, pipeline)
    ...
    assert command == (
        ...
        f"{deploy_root}/automation/deploy-skill.sh",
        "auto-draft",
    )
...
def test_dispatch_when_remote_deploy_script_is_not_executable_then_invokes_it_with_bash() -> None:
    ...
    # Then: Bash interprets an agent-read staged copy without direct runtime-root access.
