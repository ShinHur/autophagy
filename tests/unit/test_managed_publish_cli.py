...
_PUBLISHER_EMAIL = "publisher@example.invalid"
_PUBLISH_ENV = {
    "MANAGED_PUBLISHER": _PUBLISHER,
    "MANAGED_PUBLISHER_EMAIL": _PUBLISHER_EMAIL,
}


@pytest.fixture(autouse=True)
def _publisher_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MANAGED_PUBLISHER", _PUBLISHER)
    monkeypatch.setenv("MANAGED_PUBLISHER_EMAIL", _PUBLISHER_EMAIL)
...
def test_publish_when_approved_then_writes_v1_manifest_signs_tag_and_pushes(tmp_path: Path) -> None:
    ...
    manifest = publish(config, runner, _PUBLISH_ENV)
    ...
    assert f"user.email={_PUBLISHER_EMAIL}" in tag_call
...
def test_publish_when_gate_check_is_pending_then_no_managed_repo_mutation_occurs(tmp_path: Path) -> None:
    ...
    with pytest.raises(PublishError):
        publish(config, runner, _PUBLISH_ENV)
...
def test_publish_when_gate_check_precedes_approval_then_mutations_follow_it(tmp_path: Path) -> None:
    ...
    _ = publish(config, runner, _PUBLISH_ENV)
...
def test_publish_when_skill_is_outside_managed_namespace_then_refuses_before_gate(tmp_path: Path, skill: str) -> None:
    ...
    with pytest.raises(PublishError):
        publish(invalid, runner, _PUBLISH_ENV)
...
def test_publish_when_signing_key_path_is_missing_then_refuses_before_gate(tmp_path: Path) -> None:
    ...
    with pytest.raises(PublishError):
        publish(missing_key, runner, _PUBLISH_ENV)
...
def test_publish_when_staged_then_finalized_with_same_inputs_then_manifest_hash_is_deterministic(
    ...
    _ = publish(staged, FakeRunner(), _PUBLISH_ENV)
    ...
    _ = publish(finalized, runner, _PUBLISH_ENV)
...
def test_publish_when_token_file_is_provided_then_only_gate_subprocess_environment_receives_it(tmp_path: Path) -> None:
    ...
    _ = publish(config, runner, _PUBLISH_ENV | {"PUBLISHER_CONTEXT": "workstation"})
    ...
    assert gate_environments == [
        _PUBLISH_ENV | {"PUBLISHER_CONTEXT": "workstation", "DISCORD_BOT_TOKEN": "fake-gate-token"},
        _PUBLISH_ENV | {"PUBLISHER_CONTEXT": "workstation", "DISCORD_BOT_TOKEN": "fake-gate-token"},
    ]
