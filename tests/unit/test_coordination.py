...
def _query(intent: str, payload: dict) -> InteropEnvelope:
    return InteropEnvelope("coord-test", "agent-synthetic", "peer-synthetic", intent, payload)
...
def test_response_for_offers_deterministic_hour_aligned_slots() -> None:
    ...
    first = response_for(query, sender_id="peer-synthetic")
    second = response_for(query, sender_id="peer-synthetic")
...
def test_response_for_without_range_keeps_legacy_gate_marker() -> None:
    response = response_for(_query("query_availability", {"duration_min": 1}), sender_id="peer-synthetic")
    assert response.payload == {"slots": ["interop-ready"]}


def test_response_for_confirm_slot_accepts_and_echoes_slot() -> None:
    ...
    assert response.payload == {"result": "accepted", "slot": BASE.isoformat()}
...
def test_response_for_slot_offer_caps_at_eight() -> None:
    ...
    slots = response_for(query, sender_id="peer-synthetic").payload["slots"]
