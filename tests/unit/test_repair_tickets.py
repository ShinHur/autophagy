...
class FakePrivateLogs:
    ...
    def write(self, ticket_id: str, occurrence: int, raw_log: str) -> str:
        ...
        return str(Path(synthetic_env()["AUTOPHAGY_PRIVATE_ROOT"]) / "repair-logs" / ticket_id / f"occurrence-{occurrence}.log")
...
def test_ticket_excerpt_and_comment_never_include_sensitive_error_text(tmp_path: Path) -> None:
    ...
    assert str(Path(synthetic_env()["AUTOPHAGY_PRIVATE_ROOT"]) / "repair-logs" / "t_repair01") in visible
