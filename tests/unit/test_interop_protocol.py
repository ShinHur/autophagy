...
def test_mask_summary_when_secret_and_id_shapes_present_then_masks_them() -> None:
    # Given
    channel_id = fake_snowflake(27)
    raw = f"deployed with token sk-fixture-token-1 and channel {channel_id}"

    # When
    masked = mask_summary(raw)

    # Then
    assert "sk-fixture-token-1" not in masked
    assert channel_id not in masked
...
def test_report_when_status_is_not_enum_value_then_parser_returns_none() -> None:
    # Given
    malformed = """```json
{"version":"v0","agent_id":"agent-synthetic","task_id":"task-123","status":"running","summary":"x","links":[],"timestamp":"2026-07-15T14:30:00+09:00"}
```"""
...
def test_delegation_when_query_is_received_then_response_preserves_correlation_id() -> None:
    ...
    query = InteropEnvelope(
        ...
        sender_id="agent-synthetic",
        recipient_id="peer-synthetic",
        intent="query_availability",
        payload={"duration_min": 30},
    )

    # When
    response = response_for(query, sender_id="peer-synthetic")
