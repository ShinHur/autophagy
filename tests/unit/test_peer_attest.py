...
from tests.unit._synthetic import OWNER_ID, synthetic_env
...
def _tmp_checkout(tmp_path: Path) -> Path:
    """Copy the 4 stdlib-only attestor modules into a standalone tmp checkout with clean perms."""
    repo = tmp_path / "repo"
    (repo / "automation").mkdir(parents=True)
    src = Path(peer_attest.__file__).parent
    for name in ("__init__.py", "config_env.py", "peer_attest.py", "peer_attestation.py", "skill_review.py"):
...
def test_main_when_checkout_clean_but_not_runtime_then_exits_2_with_runtime_fatal(tmp_path: Path) -> None:
    # Given: a standalone checkout with clean permissions, outside the trusted runtime.
    repo = _tmp_checkout(tmp_path)
    ...
    assert completed.returncode == 2
    assert f"must run from the release runtime or {synthetic_env()['AUTOPHAGY_DEPLOY_ROOT']}" in completed.stderr
...
def test_is_trusted_attestor_root_accepts_the_ops_mirror(tmp_path: Path) -> None:
    # The resident deploy checkout stays a trusted attestor root.
    assert peer_attest._is_trusted_attestor_root(tmp_path / "ops", ops_repo_root=tmp_path / "ops")
