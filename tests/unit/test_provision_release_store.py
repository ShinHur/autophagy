...
The provisioner mirrors provision-readonly-skills.sh: create the release-store root
...
def test_provision_is_idempotent(tmp_path: Path) -> None:
    ...
    # Exactly one helper file, not duplicated by the second run.
    assert len(list(helper_dir.iterdir())) == 1
...
def test_provisioned_helper_installs_the_canonical_release_layout(tmp_path: Path) -> None:
    ...
    # INSTALLED helper with a prefixed store root and assert it lands the
    # canonical autophagy-agent-releases/<sha> + autophagy-agent-current, never the
    # generic releases + current layout.
    import io
    import tarfile
