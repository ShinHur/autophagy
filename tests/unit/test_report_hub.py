...
REGISTRY = PeerRegistry(
    peers=(
        Peer(agent_id=AGENT_ID, bot_user_id=AGENT_BOT_ID, bot_name="Autophagy-Agent"),
        Peer(agent_id=PEER_ID, bot_user_id=PEER_BOT_ID, bot_name="Autophagy-Peer"),
    )
)


def _conformant_message(agent_id: str = AGENT_ID) -> str:
...
def test_classify_when_registered_author_claims_foreign_agent_id_then_unregistered() -> None:
    # Given: the registered agent bot forges the peer agent id
    forged = _conformant_message(agent_id=PEER_ID)
...
def _row(message_id: str, agent_id: str, status: str, registered: bool = True) -> ReportRow:
    return ReportRow(
        ...
        channel_id=fake_snowflake(33),
        author_id=AGENT_BOT_ID if agent_id == AGENT_ID else PEER_BOT_ID,
...
def test_store_upsert_when_same_message_id_twice_then_single_row(tmp_path: Path) -> None:
    ...
    # When
    store.upsert_report(_row("1", AGENT_ID, "start"))
    store.upsert_report(_row("1", AGENT_ID, "done"))
    store.close()
...
def test_store_query_when_filtered_by_agent_and_status_then_matching_rows(tmp_path: Path) -> None:
    ...
    store = ReportStore(database)
    store.upsert_report(_row("1", AGENT_ID, "done"))
    store.upsert_report(_row("2", PEER_ID, "done"))
    store.upsert_report(_row("3", AGENT_ID, "blocked"))
    store.close()
    ...
    # Then
    assert {row.agent_id for row in query.reports(agent_id=AGENT_ID)} == {AGENT_ID}
    assert len(query.reports(agent_id=AGENT_ID)) == 2
    assert [row.message_id for row in query.reports(agent_id=AGENT_ID, status="done")] == ["1"]
    assert dict(query.counts_by_agent()) == {AGENT_ID: 2, PEER_ID: 1}
...
def test_store_watermark_when_set_then_survives_reopen(tmp_path: Path) -> None:
    ...
    # When
    store.set_watermark(fake_snowflake(34))
    store.close()

    # Then
    reopened = ReportStore(database)
    assert reopened.watermark() == fake_snowflake(34)
    reopened.close()
...
def test_dashboard_render_when_agent_filter_then_only_that_agents_rows(tmp_path: Path) -> None:
    ...
    store = ReportStore(database)
    store.upsert_report(_row("1", AGENT_ID, "done"))
    store.upsert_report(_row("2", PEER_ID, "start", registered=False))
    store.close()
    query = ReportQuery(database)

    # When
    filtered = render_page(query, agent_filter=AGENT_ID, status_filter="")
    unfiltered = render_page(query, agent_filter="", status_filter="")
    query.close()

    # Then
    assert f'data-agent="{AGENT_ID}"' in filtered
    assert f'data-agent="{PEER_ID}"' not in filtered
    assert f'data-agent="{PEER_ID}"' in unfiltered
...
def test_load_registry_when_valid_file_then_lookup_by_bot_id(tmp_path: Path) -> None:
    ...
    # Then
    assert registry.agent_id_for(AGENT_BOT_ID) == AGENT_ID
