...
from tests.unit._synthetic import NODE_A, synthetic_env

_REPO = Path(__file__).resolve().parents[2]
_DEPLOY = _REPO / "automation" / "deploy-skill.sh"
_DEPLOY_ROOT = synthetic_env()["AUTOPHAGY_DEPLOY_ROOT"]
_SKILLS_ROOT = str(Path(synthetic_env()["AUTOPHAGY_SKILLS_ROOT"]).parent)
...
def _run_scan(tmp_path: Path, *, checker_out: str, checker_rc: int, strict: bool = False) -> subprocess.CompletedProcess[str]:
    ...
    env["DEPLOY_SSH_HOST"] = f"{NODE_A}-not-this-host"
    if strict:
