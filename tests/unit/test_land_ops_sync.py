...
* **release mode** (the release-current path is a live symlink) — the
...
_DEPLOY_ROOT = synthetic_env()["AUTOPHAGY_DEPLOY_ROOT"]
_SERVICE_ROOT = str(Path(_DEPLOY_ROOT).parent)
...
def _enable_release(tmp_path: Path, node: Path, sha: str, *, converge: str = "ok") -> None:
    ...
    race = (
        f'race="$(mktemp -d)"; git clone -q "{origin}" "$race/c"\n'
...
def _stub_bin(
    ...
    if ssh_down:
        ...
        ssh_body = (
            ...
            f'printf "remote %s\\n" "$*" >> "{journal}"\n'
            'remote="${*: -1}"\n'
        f'remote="${{remote//{_SERVICE_ROOT}/{node}}}"\n'
            'exec bash -c "$remote"\n'
        )
...
def _run_land(
    ...
    env["AUTOPHAGY_DEPLOY_ROOT"] = str(_mirror(node))
    env["AUTOPHAGY_SKILLS_ROOT"] = str(_current(node))
    env["DEPLOY_SSH_HOST"] = "node-a-not-this-host"
    return subprocess.run(
...
def test_runtime_root_is_resolved_node_side_not_on_the_workstation() -> None:
    # A workstation-side resolver would stat the release runtime on the
    ...
    assert f"{_DEPLOY_ROOT}/automation/skill_library_abi.py" not in abi_line
    assert f"{Path(_DEPLOY_ROOT).with_name('autophagy-agent-current')}/automation/skill_library_abi.py" not in abi_line
