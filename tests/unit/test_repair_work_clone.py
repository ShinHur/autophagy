...
def test_config_when_work_clone_is_unset_then_fails_closed(monkeypatch) -> None:
    ...
    # When / Then: the ops CLI cannot select a mutation checkout implicitly.
    with pytest.raises(config_env.ConfigError, match="REPAIR_WORK_CLONE"):
        repair_ops_cli._config(["t-repair-1"])
...
def test_agent_when_constructed_then_routes_writes_to_work_clone_and_sandbox_reads_deploy(
    ...
    monkeypatch.setattr(repair_ops_cli, "planner_for", lambda _config: None)
