...
def test_extract_reply_address() -> None:
    assert triage_core.extract_reply_address("Example Person <a.b@example.invalid>") == "a.b@example.invalid"
...
def _draft(*, sensitive: bool) -> dict:
    ...
    body = f"본문 {CANARY}"
    argv = site_mail_backend.build_send_argv(
        site_mail_backend.SendRequest(
            to=to,
            subject=subject,
            body=body,
            attachments=(),
    ...
    record = {
        ...
        "provider": "site",
        "site_backend_config_sha256": BACKEND_HASH,
    }
...
def test_sensitive_approval_rendering_confines_full_text_to_owner_dm() -> None:
    ...
    assert "sender@example.invalid" not in owner_dm and "owner@example.invalid" not in owner_dm
...
def test_non_sensitive_approvals_message_shows_reply_text() -> None:
    ...
    assert "sender@example.invalid" not in message
...
def test_direct_site_bridge_send_matches_denylist() -> None:
    rules = external_effect_gate.load_denylist(_REPO / "configs" / "external-effect-tools.yaml")
    for command in (
        shlex.join(tuple(_draft(sensitive=False)["argv"])),
        "python3 /opt/mail/site_mail_backend.py send --request-json {}",
    ):
    ...
    read_call = external_effect_gate.ToolCall(
        tool_name="python3",
        arguments={"command": "python3 /opt/mail/site_mail_backend.py status --request-json '{}'"},
    )
    context = external_effect_gate.ApprovalContext(
        approval_log=None, owner_id="o", e2e_test_mode=False
    )
...
def test_downgrade_writes_runtime_no_go_and_switch_record(
    ...
    triage_mode.downgrade_to_no_go("approved site mail send failed 2 consecutive times")
...
def test_repo_mode_file_default_is_the_seed(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("AUTOPHAGY_DEPLOY_ROOT", "/opt/autophagy")
    assert triage_mode.repo_mode_file() == Path("/opt/autophagy/configs/mail-mode.default.json")
...
def test_mask_value_shape_and_determinism() -> None:
    masked = triage_core.mask_value("발신자 <person-alpha@example.invalid>")
    assert masked.startswith("sha256:") and len(masked) == len("sha256:") + 16
    assert masked == triage_core.mask_value("발신자 <person-alpha@example.invalid>")


def test_redact_hides_emails_and_long_digits() -> None:
    redacted = triage_core.redact("fail person-alpha@example.invalid code 1234567890")
    assert "person-alpha@example.invalid" not in redacted and "1234567890" not in redacted


def test_send_argv_shape_matches_site_bridge_contract() -> None:
    argv = tuple(_draft(sensitive=False)["argv"])
    assert argv[1].endswith("site_mail_backend.py")
    assert argv[2] == "send"
    assert argv[argv.index("--expected-config-sha256") + 1] == BACKEND_HASH
