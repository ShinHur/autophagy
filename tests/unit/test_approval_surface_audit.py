...
from tests.unit._synthetic import APPROVALS_CHANNEL_ID
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (audit.Flow.DRIVE, "drive_archive"),
    ),
)
...
def test_empty_root_reports_zeroes(tmp_path: Path) -> None:
    ...
    # When: it is audited.
    result = audit.audit_flow(audit.Flow.DRIVE, root)
...
def test_audit_module_has_no_write_or_network_primitives() -> None:
    ...
    assert all(token not in source for token in ("write_text", "url" + "open", "mkdir", "unlink"))
