...
def test_peer_sandbox_when_building_staged_command_then_chains_shell_statements(tmp_path: Path) -> None:
    ...
    sandbox = PeerSandbox("node-a", tmp_path)
...
def test_manual_approval_when_reactor_is_bot_then_rejects(tmp_path: Path) -> None:
    ...
    gate = ManualOwnerApproval("owner", ApprovalReaction("m1", "owner", "approvals", f"APPROVE repair {action_hash} ticket:t_repair01", True))
...
def test_signed_injection_when_owner_hmac_matches_then_writes_approval_record(tmp_path: Path) -> None:
    ...
    action_hash = hashlib.sha256(b"repair:t_repair01:patch.diff").hexdigest()
    secret = b"1" * 32
    event = InboundEvent("e1", "owner", "approvals", f"APPROVE repair {action_hash} ticket:t_repair01")
    gate = SignedOwnerApproval("owner", tmp_path / "approvals.jsonl", event, sign_event(event, secret), secret, True)
