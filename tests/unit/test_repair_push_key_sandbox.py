...
def test_default_push_key_when_units_protect_home_then_key_lives_outside_it() -> None:
    ...
    default = str(cli._default_push_key())
...
def test_default_push_key_when_chosen_then_sits_in_the_private_state_root() -> None:
    ...
    # under the configured private state root (or ~/.hermes) — never in a tracked path.
    assert str(cli._default_push_key()).startswith(f"{synthetic_env()['AUTOPHAGY_PRIVATE_ROOT']}/")
...
def test_default_known_hosts_when_units_protect_home_then_lives_outside_it() -> None:
    ...
    assert not str(cli._default_known_hosts()).startswith(_SHADOWED)
    assert str(cli._default_known_hosts()).startswith(f"{synthetic_env()['AUTOPHAGY_PRIVATE_ROOT']}/")
