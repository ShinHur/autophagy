...
def draft_gate_dir(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    peers.write_text(
        ...
        "peers:\n"
        "  agent-synthetic:\n"
        "    bot_user_id: \"100000000000000005\"\n"
        "    bot_name: agent-synthetic\n"
        "    account: agent\n"
        "  peer-synthetic:\n"
        "    bot_user_id: \"100000000000000006\"\n"
        "    bot_name: peer-synthetic\n"
        "    account: peer\n",
        encoding="utf-8",
    )
...
def test_draft_create_peer_named_window_with_cue_refuses_to_coordination(
    ...
    args = argparse.Namespace(
        text="peer-synthetic와 다음주 오전에 가능한 시간 조율해줘", summary="",
...
def test_draft_create_peer_named_with_exact_time_creates_solo_draft(
    ...
    args = argparse.Namespace(
        text="peer-synthetic랑 다음주 수요일 오전 10시 30분 미팅", summary="",
        calendar="primary", channel_id="dm",
    )
...
def test_draft_create_bare_peer_name_without_cue_clarifies_without_draft(
    ...
    args = argparse.Namespace(
        text="peer-synthetic 다음주 미팅", summary="", calendar="primary", channel_id="dm"
    )
...
def test_draft_create_when_request_names_owner_agent_creates_draft(draft_gate_dir: Path) -> None:
    ...
    args = argparse.Namespace(
        text="agent-synthetic와 내일 오후 3시 실험 미팅", summary="", calendar="primary", channel_id="dm"
    )
