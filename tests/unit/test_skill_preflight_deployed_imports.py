...
def test_repo_root_skips_candidates_without_automation(
    ...
    A mounted skill runs from the release-store ``releases/<skill>/<hash>/scripts`` path,
    ...
    refused every send because the resolved root lacked ``automation``,
    ...
    with monkeypatch.context() as isolated:
        ...
        isolated.syspath_prepend(str(_SKILLS_ROOT / skill_name / "scripts"))
        isolated.syspath_prepend(str(_REPO_ROOT))
        importlib.invalidate_caches()
        preflight = importlib.import_module(module_name)
    ...
    assert resolved == Path(synthetic_env()["AUTOPHAGY_DEPLOY_ROOT"]), (
