...
def _gmail_draft() -> dict[str, JsonValue]:
    action = mail_gmail_send.build_action(
        mail_gmail_send.NewMailRequest(
            ...
            to="PersonA <recipient@example.test>",
            subject="PersonA 검토 요청",
            body="PersonA에게 최종본을 보내세요.",
        )
    )
...
def _request(
    ...
    *,
    ...
    entities = tuple(
        DetectedEntity(f"m-{index}", "PersonA", EntityKind.PERSON, start, start + len("PersonA"))
        for index, start in enumerate(_positions(raw_text, "PersonA"), 1)
...
def test_mail_preflight_when_high_confidence_candidate_then_normalizes_final_gmail_snapshot_before_send(
    ...
    source = _FixtureSource(tuple(_candidate(mention_id, "PersonB", 0.98) for mention_id in mentions))
    ...
    assert snapshot.recipients == "PersonB <recipient@example.test>"
    assert snapshot.subject == "PersonB 검토 요청"
    assert snapshot.body == "PersonB에게 최종본을 보내세요."
    assert approval_record["approval_snapshot"]["recipients"] == "PersonB <recipient@example.test>"
...
def test_mail_preflight_when_candidates_conflict_then_clarifies_once_without_send(
    ...
    candidates = tuple(
        ...
        for candidate in (_candidate(mention_id, "PersonB", 0.98), _candidate(mention_id, "PersonC", 0.97))
    )
    ...
    assert "PersonB" in str(first.value) and "PersonC" in str(first.value)
...
def test_mail_cli_when_candidates_conflict_then_prints_one_clarify_and_exits_nonzero(
    ...
    assert "PersonB" in captured.out and "PersonC" in captured.out
