...
from tests.unit._synthetic import OWNER_DM_CHANNEL_ID  # noqa: E402

OWNER_ID = "owner-1"
POSTED_MESSAGE_ID = "m-9"
NOW = datetime(2026, 7, 25, 9, 0, tzinfo=UTC)
REPAIR_BINDING = ApprovalBinding(
    ApprovalKind.REPAIR, ApprovalSurface.OWNER_DM, OWNER_DM_CHANNEL_ID, POLICY_VERSION
)
BACKEND_HASH = "sha256:" + "d" * 64
...
MAIL_DRAFT_KEYS = [
    "argv", "body", "category", "channel_id", "created", "flags", "id", "kind",
    "mail_subject", "message_id", "policy_version", "provider", "sender", "sender_masked",
    "sensitive", "sha256", "site_backend_config_sha256", "status", "subject", "surface",
    "tags", "to", "uid", "uid_opaque",
]
BUDGET_DRAFT_KEYS = [
...
def _mail_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr(
        triage_gate.site_mail_backend, "config_sha256", lambda: BACKEND_HASH
    )
    monkeypatch.setattr(
        triage_gate.site_mail_backend, "verify_config_sha256", lambda _expected: None
    )
