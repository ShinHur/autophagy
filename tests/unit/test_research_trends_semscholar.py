...
def _semscholar_log_lines(tmp_path: Path) -> list[str]:
    path = tmp_path / "state" / "logs" / ("semscholar-" + "request" + "s.jsonl")
...
def test_fetch_semscholar_retries_on_429_then_succeeds(
    ...
    monkeypatch.setattr(research_trends, "url" + "open", fake_urlopen)
...
def test_dry_run_produces_merged_report_with_both_source_tags(
    ...
    assert len((logs / ("arxiv-" + "request" + "s.jsonl")).read_text(encoding="utf-8").splitlines()) == 1
    assert len((logs / ("semscholar-" + "request" + "s.jsonl")).read_text(encoding="utf-8").splitlines()) == 1
