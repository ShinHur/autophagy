...
from automation.interop.approval_surface import POLICY_VERSION, ApprovalKind, ApprovalSurface, reaction_instruction
...
TICKET = "t_repair01"
NONCE = "n" * 32
SOURCE = "fixtures/repair-plans/t_repair01/patch.diff"
...
def test_legacy_record_renders_the_frozen_v1_message_shape() -> None:
    # Given: a record written before content binding existed.
    legacy = _Record()

    # When: its already-posted Discord message is rendered.
    content = approval_request_content(legacy)

    # Then: the frozen request fields remain intact so migration cannot paralyse the gate.
    assert content.startswith(V1_GOLDEN)
    assert f"- sha256: `{LEGACY_HASH}`" in content
    assert f"- repair_nonce: `{NONCE}`" in content
    assert content.endswith(f"{reaction_instruction(ApprovalKind.REPAIR, ApprovalSurface.OWNER_DM)} 리액션")
...
def test_v2_shows_the_digest_the_totals_the_per_file_deltas_and_the_private_path() -> None:
    ...
    # Then: everything the owner needs to consent to the change is present.
...
def test_v2_keeps_long_unicode_paths_within_the_discord_limit() -> None:
    ...
    record = _v2(changes=long_paths, ticket_id="t_" + "x" * 200, patch_source_path="fixtures/" + "p" * 400)
