...
`<organization> 산출물 / procurement / <YYYY-MM> / <file>` — the review-DM
...
from tests.unit._synthetic import ORG_LABEL
...
def test_drive_upload_targets_dated_project_folder(
    ...
    monkeypatch.setenv("ORGANIZATION_LABEL", ORG_LABEL)
    calls: list[list[str]] = []
    responses = iter(
        [
    ...
    assert [c["name"] for c in creates] == [f"{ORG_LABEL} 산출물", "procurement", "2026-07"]
