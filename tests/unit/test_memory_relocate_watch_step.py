...
class FakeEffects:
    ...
    post_results: list[str | None] = field(default_factory=lambda: ["approval-1"])
    verdict: str = "pending"
    ...
    def post_approval(self, record: RelocationRecord) -> str | None:
...
def test_resolve_tick_when_owner_cancels_abandons_record() -> None:
    # Given: the owner cancelled a posted, bound approval.
...
def test_resolve_tick_when_three_are_proposed_posts_only_first_sorted_key() -> None:
    ...
    fake = FakeEffects(post_results=["approval-a", "approval-b", "approval-c"])
...
def test_resolve_tick_when_record_is_already_posted_never_reposts_or_overwrites_message_id() -> None:
    ...
    record = _posted_record()
    fake = FakeEffects(post_results=["approval-replacement"], verdict="pending")
