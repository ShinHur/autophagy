...
_ENTRY = "NODE_A 노드가 prod이고 NODE_B는 개인 RAG 전용이다."
...
def test_relocation_plan_lands_in_personal_para_resource_when_planned() -> None:
    ...
    # Then: the note lands under the personal PARA Resource bucket, not the organization root.
...
def test_note_plan_sha256_changes_when_the_entry_changes() -> None:
    ...
    first = build_relocation_plan("NODE_A 노드가 prod이다.")
    second = build_relocation_plan("NODE_A 노드가 prod이다!")
