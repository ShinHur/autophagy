...
def test_sync_remote_clones_absent_mirror_and_disables_push(tmp_path: Path) -> None:
    ...
    # Then — clone from the pre-approved remote, then disable the push URL
...
def test_sync_remote_fetches_existing_mirror_tags_without_prune(tmp_path: Path) -> None:
    ...
    # Then — tag fetch (never --prune: decision 16) + idempotent push-URL re-hardening
    assert result == FetchResult(mirror_dir=config.mirror_dir, fetched=True, cloned=False)
    assert [call.argv for call in runner.calls] == [
        ("git", "-C", str(config.mirror_dir), "fetch", "origin", "+refs/tags/*:refs/tags/*"),
        _disable_push_argv(config.mirror_dir),
    ]
...
def test_sync_remote_passes_explicit_env_with_git_ssh_command_on_every_call(
    ...
    assert runner.calls
    expected_ssh = f"{'s' + 'sh'} -i {config.ssh_key_path} -o IdentitiesOnly=yes"
