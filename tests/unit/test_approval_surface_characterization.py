...
All seven flows — mail, budget, patent-export, repair, drive-archive,
...
All five migrating flows — mail reply, budget, patent-export, repair and
drive-archive — now land on the owner DM alongside mail compose, calendar,
...
@pytest.mark.parametrize(
    ("kind", "expected_hash"),
    (("reply", EXPECTED_CANONICAL_HASHES[0]), ("compose", EXPECTED_CANONICAL_HASHES[1])),
    ids=("reply", "compose"),
)
def test_mail_draft_hash_stays_canonical(
    kind: str,
    expected_hash: str,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _bind_mail(tmp_path, monkeypatch)
    draft = _mail_draft(kind)

    assert draft["sha256"] == expected_hash
...
def test_drive_archive_watcher_reads_the_channel_off_the_stored_record() -> None:
    # Given: the reaction watcher's per-batch step and the transport it binds.
    relative = "automation/drive_archive/confirm_reaction_watch.py"

    # When / Then: the request it builds names the channel the RECORD stored, and
    # the poll is re-pointed at that same channel — the "approvals" literal the
    # watcher used to invent is gone...
    assert _channel_keyword_source(relative, "_process") == "pending.channel_id"
    assert _return_sources(relative, "_bound") == frozenset({
        "self.transport",
        "replace(self.transport, channel_id=pending.channel_id)",
    })
    # ...and no resolver is reachable from the reaction read: a tick that re-asked
    # the directory could poll a channel the approval message never lived in.
    assert _called_names(_definition(relative, "_process")) == frozenset({
        "ApprovalRequest", "PendingDecision", "_bound", "_discard", "_parse_created",
        "approval_key", "astimezone", "hold", "now", "resolve_owner_decision",
    })
