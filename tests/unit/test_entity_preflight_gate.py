...
_RAW: Final = "가상사용자-나한테 전화하라고 할 일 추가"
_NORMALIZED: Final = "가상사용자-다"
...
def _request(candidates: tuple[ResolutionCandidate, ...], *, no_entity: bool = False) -> GuardRequest:
    ...
    if not no_entity:
        entity = DetectedEntity("m-1", "가상사용자-나", EntityKind.PERSON, 0, len("가상사용자-나"))
        entities = (entity,)
        queries = (RelationshipQuery("q-1", entity.mention_id, "family_member", EntityKind.PERSON, "가상사용자-나 가족"),)
...
def test_guard_normalizes_once_writes_once_and_records_private_audit() -> None:
    ...
    assert writer.writes == [{"title": "가상사용자-다한테 전화하라고 할 일 추가"}]
    ...
    assert "가상사용자-나" not in operational
...
def test_guard_clarifies_once_and_never_starts_external_write() -> None:
    ...
    request = _request((_candidate("rag-a", "가상사용자-다", 0.98), _candidate("rag-b", "가상사용자-라", 0.97)))
    writer = FakeWriter()
    audit = AuditSink()
    log = OperationalLog()
    ...
    assert "가상사용자-다" in str(first.value)
    assert "가상사용자-라" in str(first.value)
