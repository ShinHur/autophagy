...
import pytest
from tests.unit._synthetic import APPROVALS_CHANNEL_ID, OWNER_DM_CHANNEL_ID
...
DM_CHANNEL = OWNER_DM_CHANNEL_ID
...
def budget_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config.write_text(
        json.dumps({"owner_id": OWNER_ID, "personal_approvals_channel_id": APPROVALS_CHANNEL}),
        encoding="utf-8",
    )
...
def test_watch_sends_once_when_owner_approves(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Then: the frozen draft is sent exactly once and retained from discard
    assert len(sent) == 1
    assert discarded == [] and notices == []
...
def test_watch_discards_when_owner_cancels(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert notices == ["과제비 요청 메일 발송 취소됨"]
...
def test_watch_preadds_approve_then_cancel_reactions(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [path for method, path in requests if method == "PUT"] == [
        f"/channels/{DM_CHANNEL}/messages/{MESSAGE_ID}/reactions/%E2%9C%85/@me",
        f"/channels/{DM_CHANNEL}/messages/{MESSAGE_ID}/reactions/%E2%9B%94/@me",
    ]
...
@pytest.mark.usefixtures("budget_env")
def test_request_posts_to_the_owner_dm(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Then: the single approval message lands in the owner's DM, never the guild channel,
    ...
    assert posts == [f"/channels/{DM_CHANNEL}/messages"]
    assert reaction_instruction(ApprovalKind.BUDGET_MAIL, ApprovalSurface.OWNER_DM) in posted[0]
