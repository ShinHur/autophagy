...
def test_single_high_confidence_candidate_is_auto_selected() -> None:
    raw = "가상사용자-가 기관의 공개행사를 일정에 추가해줘"
    request = _request(raw)
    result = decide(
        request,
        (_candidate("c-1", "가상기관-가", 0.98, SourceKind.PERSONAL_RAG),),
        _audit(raw),
        _POLICY,
    )
    ...
    assert result.selected[0].normalized_value == "가상기관-가"
...
def test_conflicting_candidates_require_confirmation() -> None:
    ...
    result = decide(
        request,
        (
            _candidate("c-1", "가상기관-가", 0.96, SourceKind.ADDRESSBOOK_CONTACTS),
            _candidate("c-2", "가상기관-나", 0.92, SourceKind.ADDRESSBOOK_ORGANIZATION),
        ),
        _audit(raw),
        _POLICY,
    )
...
def test_operational_event_contains_no_personal_raw_values() -> None:
    ...
    result = decide(
        _request(raw),
        (_candidate("c-1", "가상기관-가", 0.98, SourceKind.HERMES_MEMORY),),
        _audit(raw),
        _POLICY,
    )

    rendered = json.dumps(operational_event(result), ensure_ascii=False)
    for forbidden in ("가상사용자-가", "가상기관-가", "다니는 기관", "private://"):
...
def test_unresolved_entity_is_clarified_in_conversation_not_by_approval() -> None:
    ...
    assert rendered.index("가상기관-가") < rendered.index("가상기관-나")
...
def test_operational_event_reports_clarify_need_without_personal_values() -> None:
    ...
    assert "가상사용자-가" not in json.dumps(event, ensure_ascii=False)
