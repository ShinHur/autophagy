...
class RelationshipAwareSource:
    ...
    def resolve(
        ...
        if query is None:
            return ()
        required_signals = ("가상사용자-아", "가상사용자-하", "family_member", "send")
...
def test_normalizes_voice_transcription_when_relationship_query_finds_one_high_candidate() -> None:
    ...
    raw_text = "가상사용자-아에게 메일을 보내줘"
    candidate = _candidate(CandidateSpec("rag-synthetic", SourceKind.PERSONAL_RAG, "가상사용자-하", 0.98))
    ...
    decision = resolve_preflight(_request(raw_text, "가상사용자-아"), _audit(raw_text), (source,))
    ...
    assert decision.selected[0].normalized_value == "가상사용자-하"
