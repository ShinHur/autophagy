...
def _site_argv(body: str) -> tuple[str, ...]:
    return triage_gate.site_mail_backend.build_send_argv(
        triage_gate.site_mail_backend.SendRequest(
            to=COMPOSE_TO,
...
def test_compose_draft_record_binds_kind_channel_and_frozen_argv(
    ...
    assert record["argv"] == list(_site_argv(COMPOSE_BODY))
