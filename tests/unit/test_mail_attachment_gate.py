...
def _env(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(triage_gate.site_mail_backend, "config_sha256", lambda: _BACKEND_HASH)
    monkeypatch.setattr(
...
def test_no_attachment_argv_uses_the_stable_bridge(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    _env(monkeypatch, tmp_path)
    draft = _draft(tmp_path)
    request = json.loads(draft["argv"][draft["argv"].index("--request-json") + 1])
    assert draft["argv"][1].endswith("site_mail_backend.py")
    assert draft["argv"][2] == "send"
...
def test_multi_attachment_manifest_is_hash_bound_and_rendered_safely(
    ...
    request = json.loads(draft["argv"][draft["argv"].index("--request-json") + 1])
    assert [item["filename"] for item in request["attachments"]] == ["same.txt", "same.txt"]
