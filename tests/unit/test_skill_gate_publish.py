...
_APPROVE_LINE = skill_gate_specs._APPROVAL_LINE
...
def test_provenance_lines_moved_to_skill_gate(tmp_path: Path) -> None:
    ...
    path.write_text(
        json.dumps({"publisher": "publisher-synthetic-9876", "tag": _TAG, "release_sequence": 1, "manifest_sha256": _MANIFEST_DIGEST}),
        encoding="utf-8",
    )
    line = skill_gate.provenance_lines(path)
    assert line.startswith("\n- provenance: publisher \u2026")
    assert "publisher-synthetic-9876" not in line
    assert _TAG in line
