...
적재하는 파이프라인. **임베딩은 RAG 노드(`node-b`) 내부 로컬 bge-m3 서버에서만**
...
| `peer-report` | #agents-log 의 타 에이전트 W1-6 v0 보고(정확 7키 JSON 블록) | `agents-log:<message_id>` |
...
## 배포 (agent@`node-a`, 에이전트 노드 — `DEPLOY_SSH_HOST`)
