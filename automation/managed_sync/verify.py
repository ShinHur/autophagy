...
_VERIFIED_PRINCIPAL: Final = re.compile(r'Good "git" signature for (\S+) with ')
...
def _verify_signature(git: _Git, mirror: Path, tag: str, config: VerifyConfig) -> None:
    result = git.run(
        (
            ...
            str(mirror),
            "-c",
            f"gpg.ssh.allowedSignersFile={config.allowed_signers}",
            "verify-tag",
            tag,
        ),
        "BAD-SIGNATURE",
    ...
    expected_principal = config_env.require_env(
        "MANAGED_PUBLISHER_PRINCIPAL",
        hint="Set the allowed managed-skill signing principal",
    )
    if principal is None or principal.group(1) != expected_principal:
        raise ManagedVerifyError("WRONG-PRINCIPAL", "verified principal differs from the configured publisher")
