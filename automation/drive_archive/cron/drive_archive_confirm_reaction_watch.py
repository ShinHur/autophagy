"""Hermes cron watcher (no_agent, LLM-free) for drive-archive batch approvals (E11).

Deployed to ``~/.hermes/scripts/drive_archive_confirm_reaction_watch.py`` and
registered as

    hermes cron create "every 1m" --name drive-archive-confirm-watch \
        --no-agent --script drive_archive_confirm_reaction_watch.py --deliver local

Each tick runs exactly one ``confirm_reaction_watch`` pass — it polls only the
Discord reactions on posted digests (규약 (a): never messages) and, on a bound
owner ✅, writes the approval record and runs the gated upload child. No other
subcommand, no extra flag.

No-agent cron contract (docs/guide/watcher-cron-설계규약.md):
- (b) secrets are self-loaded from ``~/.env.secrets`` (system env wins);
- (b-2) the upload child receives the credential via an explicit ``env=``;
- (c) the repo root (``AUTOPHAGY_REPO_ROOT``) is inserted into ``sys.path`` and
  exported as ``PYTHONPATH`` so ``automation.*`` imports resolve;
...
LOCK_PATH = Path.home() / ".hermes" / "drive-archive" / "watch.lock"
...
def run_watch_once() -> int:
    """Run exactly one reaction-poll/dispatch pass via the watcher module."""
