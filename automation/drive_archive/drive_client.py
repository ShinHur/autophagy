...
class DriveClient:
    ...
    def _create_folder(self, name: str, parent: str) -> str:
        ...
        if not folder_id:
            raise DriveArchiveError(f"폴더 생성 응답에 id 없음: {name}")
        return folder_id
    ...
    def _upload_new(self, local: Path, name: str, parent: str) -> str:
        ...
        if not file_id:
            raise DriveArchiveError(f"업로드 응답에 id 없음: {name}")
        return file_id
    ...
    def ensure_folder_path(self, parts: tuple[str, ...]) -> str:
        if not parts:
            raise DriveArchiveError("folder path is empty")
    ...
    def verify_owner_only(self, file_id: str) -> None:
        ...
        if not rows:
            raise DriveArchiveError(f"권한 목록이 비어 판정 불가(fail-closed): {file_id}")
        for row in rows:
            reason = _reject_reason(str(row.get("type", "")), str(row.get("role", "")))
            if reason is not None:
                raise DriveArchiveError(f"{reason}: {file_id}")
        if len(rows) != 1:
            raise DriveArchiveError(
    ...
    def download_and_verify(self, file_id: str, local: Path) -> str:
        ...
        with tempfile.TemporaryDirectory(prefix="drive-verify-") as tmp:
            ...
            if not fetched.is_file():
                raise DriveArchiveError(f"재다운로드 산출물 없음(fail-closed): {file_id}")
