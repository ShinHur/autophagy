...
def _default_push_key() -> Path:
    return config_env.private_root() / "repair_push_key"


def _default_known_hosts() -> Path:
    return config_env.private_root() / "repair_known_hosts"
...
def _config(argv: list[str]) -> RepairOpsConfig:
    ...
    return RepairOpsConfig(
        argv[0],
        Path(config_env.optional_env("REPAIR_CHECKOUT") or config_env.deploy_root()),
        Path(config_env.optional_env("REPAIR_LOG_ROOT") or config_env.private_root() / "repair-logs"),
        Path(config_env.optional_env("REPAIR_PLAN_ROOT") or config_env.private_root() / "repair-plans"),
        Path(config_env.optional_env("REPAIR_APPROVAL_LOG") or config_env.private_root() / "repair-approvals.jsonl"),
        _event_from_environment(),
        os.environ.get("REPAIR_E2E_SIGNATURE"),
        Path(config_env.require_env("REPAIR_WORK_CLONE", hint="Set an isolated repair work clone path")),
    )
...
def _approval(config: RepairOpsConfig) -> Approval | None:
    ...
    if e2e_mode == "1":
        ...
        try:
            ...
            return None
        return SignedOwnerApproval(config_env.discord_owner_id(), config.approval_log, config.event, config.signature, secret, True)
...
def _pending_root() -> Path:
    return Path(
        config_env.optional_env("REPAIR_APPROVAL_PENDING_ROOT")
        or config_env.private_root() / "repair-approval-pending"
    )
...
def planner_for(config: RepairOpsConfig) -> CodexPlanner | LiteLLMPlanner | StaticPlanner:
    ...
    return LiteLLMPlanner(
        config.plans,
        Path(config_env.require_env("REPAIR_LITELLM_KEY_FILE", hint="see .env.example")),
    )
...
def _agent(config: RepairOpsConfig, approval: Approval) -> RepairAgent:
    ...
    return RepairAgent(
        planner_for(config),
        PeerSandbox(config_env.deploy_ssh_host(), config.checkout),
        approval,
        GitRepository(
            work_clone,
        ...
        RepairLifecycleStore(
            Path(config_env.optional_env("REPAIR_STATE_ROOT") or config_env.private_root() / "repair-state")
        ),
    )
...
def _push_repair_branch(config: RepairOpsConfig, outcome: RepairOutcome) -> str | None:
    ...
    if os.environ.get("E2E_TEST_MODE") is not None:
        return None
    key = Path(config_env.optional_env("REPAIR_PUSH_KEY") or _default_push_key())
    if not key.is_file():
    ...
    known_hosts = Path(config_env.optional_env("REPAIR_KNOWN_HOSTS") or _default_known_hosts())
...
def _apply_approved(config: RepairOpsConfig) -> int:
    ...
    owner_id = config_env.discord_owner_id()
    if reaction_decision(pending, owner_id, bound_discord) is not ReactionDecision.APPROVED:
        return 1
    ...
    return _run(config, ManualOwnerApproval(owner_id, reaction, bound_discord.binding.channel_id))
...
def _discard(config: RepairOpsConfig, reason: str) -> int:
    ...
    lifecycle = RepairLifecycleStore(
        Path(config_env.optional_env("REPAIR_STATE_ROOT") or config_env.private_root() / "repair-state")
    )
