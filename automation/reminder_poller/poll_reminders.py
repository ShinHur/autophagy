...
Reads the owner's calendar through the W3-1 read-only list path (`gws calendar
events list`) and DMs the owner for any event starting 55-65 minutes out; reads
~/state/milestones.yaml (W2-3) and DMs the owner at D-3/D-1/D-day. Both legs are
...
notification to the owner's own DM — not an external effect, so no approval gate.
