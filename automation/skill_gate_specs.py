...
_APPROVAL_LINE: Final = (
    f"- 승인 방법: 이 메시지에 소유자가 {APPROVE_EMOJI} 리액션 (소유자 전용 — 봇/타인 리액션은 거부됨)"
)
...
@dataclass(frozen=True, slots=True)
class Provenance:
    """Managed-release provenance a deploy request displays to the owner."""
...
def provenance_lines(path: Path) -> str:
    ...
    return (
        ...
        f" / tag `{provenance['tag']}` / sequence {int(provenance['release_sequence'])}"
...
class DeploySpec:
    ...
    def _hash_of(self, digest: str) -> str:
        return _hash(
            "skill-deploy", self.skill, digest, self.provenance.tag, self.provenance.manifest_sha256
        )
