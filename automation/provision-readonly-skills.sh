...
readonly STORE_ROOT="/srv/autophagy-skills"
readonly LIVE_ROOT="$STORE_ROOT/live"
readonly TARGET="/home/agent/.hermes/skills"
readonly HUB_STATE="/home/agent/.hermes/skill-hub-state"
readonly HUB_TARGET="$TARGET/.hub"
readonly HELPER="/usr/local/libexec/autophagy-install-skill"
...
id agent >/dev/null 2>&1 || die "agent account missing"
...
install -d -m 0700 -o agent -g agent "$HUB_STATE"
install -m 0755 -o root -g root "$REPO_ROOT/automation/skill_store.py" "$HELPER"
install -m 0440 -o root -g root "$REPO_ROOT/automation/sudoers.d/autophagy-skill-store" "$SUDOERS"
...
sudo -n -u agent -H sh -lc 'export PATH="$HOME/.local/bin:$PATH"; hermes skills list >/dev/null'
