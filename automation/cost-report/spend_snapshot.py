...
Runs as the configured operator host. Queries the LiteLLM Postgres container
(`autophagy-litellm-postgres-1`, table "LiteLLM_SpendLogs") and:

  1. appends a FULL-detail JSON snapshot to
     the configured private runtime-log directory (dir 700, file 600,
...
PRIVATE_SUBDIR = ("runtime-logs", "cost-report")
...
def spend_window(where: str) -> dict[str, float | int]:
    rows = psql(
...
def per_alias(where: str) -> list[dict[str, object]]:
    rows = psql(
        "SELECT COALESCE(v.key_alias,'(no-alias)'), count(*)::bigint, COALESCE(SUM(s.spend),0)::float8 "
...
def build_snapshots() -> tuple[dict[str, object], dict[str, object]]:
    ...
    full: dict[str, object] = {
        ...
        "kst_now": kst_now,
        "node": config_env.deploy_ssh_host(),
        ...
        "per_key_month_kst": per_key_month,
        "per_key_all_time": per_alias(""),
    }
    fixture = os.environ.get("COST_REPORT_FIXTURE", "")
    if fixture:
        full["fixture"] = fixture

    masked_aliases: list[dict[str, object]] = []
    for entry in per_key_month:
        alias = entry["alias"] if entry["alias"] in ALIAS_WHITELIST else "other"
    ...
    if not isinstance(redacted, dict):  # pragma: no cover — redact_deep preserves dicts
        raise RuntimeError("redaction changed snapshot shape")
    return full, redacted


def private_dir() -> Path:
    """Resolve the ops-only snapshot directory from deployment configuration."""
    return config_env.private_root().joinpath(*PRIVATE_SUBDIR)


def write_private(full: dict[str, object]) -> Path:
    directory = private_dir()
    os.umask(0o077)
    directory.mkdir(mode=0o700, parents=True, exist_ok=True)
    os.chmod(directory, 0o700)
    stamp = dt.datetime.now(dt.timezone.utc).strftime("%Y%m%dT%H%M%SZ")
    path = directory / f"spend-{stamp}.json"
