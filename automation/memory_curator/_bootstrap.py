...
then the release path derived from ``AUTOPHAGY_DEPLOY_ROOT`` and its configured
mirror — the first whose
...
def _resolve_repo_root() -> Path | None:
    deploy_root = os.environ.get("AUTOPHAGY_DEPLOY_ROOT", "").strip()
    release_current = Path(deploy_root).with_name("autophagy-agent-current") if deploy_root else None
    candidates = (
        os.environ.get("AUTOPHAGY_REPO_ROOT", "").strip(),
        str(Path(__file__).resolve().parents[2]),
        str(release_current) if release_current else "",
        deploy_root,
    )
