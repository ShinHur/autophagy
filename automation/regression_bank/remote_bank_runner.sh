...
STATE_HOST="${REGRESSION_BANK_STATE_HOST:?REGRESSION_BANK_STATE_HOST is required — see .env.example}"
...
record_state() {
  local returncode="$1"
  ssh "$STATE_HOST" "sudo -n -u agent -H python3 /home/agent/.hermes/regression_bank_runtime/bank_state.py record --returncode $returncode"
}
