...
class DiscordChannelDirectory:
    ...
    def __post_init__(self) -> None:
        if not self.token or not self.owner_id:
            raise ApprovalSurfaceError("Discord directory requires a bot token and owner id")
    ...
    def _request(
        ...
        try:
            ...
            raise ApprovalSurfaceError(f"Discord request failed: {method} {path}") from error
