...
def pre_tool_call(tool_name, args, **kwargs):
    ...
    try:
        decision = evaluate_tool_call(
            ...
            ApprovalContext(
                approval_log=_external_effect_approval_log(),
...
def _external_effect_approval_log() -> Path:
    return Path(
        config_env.optional_env("EXTERNAL_EFFECT_APPROVAL_LOG")
        or config_env.deploy_root() / "logs/approvals.jsonl"
    )
