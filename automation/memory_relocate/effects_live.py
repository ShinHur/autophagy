...
def record_push_approval(
    ...
    *,
    ...
    an owner approval for THAT exact tool call.  The owner's ✅ on the relocation message already
...
def build_effects(*, memory_dir: Path, state_path: Path, rag_state_path: Path, token: str, owner_id: str, now: datetime) -> ResolveEffects:
    ...
    def post(record: RelocationRecord) -> str | None:
        try:
            text = recover_entry_text(memory_dir, record)
            if text is None:
                return None
            ...
            return None
        return None if verdict.posted is None else verdict.posted.message_id

    def probe(record: RelocationRecord) -> str:
        try:
            text = recover_entry_text(memory_dir, record)
            if text is None or record.message_id is None:
                return "pending"
