...
쓴다. 공유(Drive/repo 이동)는 자동으로 하지 않으며, 소유자의 명시적 공유 결정 후에만
별도 작업으로 수행한다.
...
2. 노트·보고서·슬라이드·대본 본문을 Discord 공개 채널, 저장소 또는 검증 증적에 붙이지
...
## Drive 게시 (최종본)
최종 산출물은 `DRIVE_PUBLISH_ENABLED=1`일 때 소유자 본인 Drive의 `Example Organization/report/<YYYY-MM>/`에 생성 즉시 자동 업로드된다(초안 제외, 리뷰용, 게이트 없음). 공통 vendored 헬퍼 `scripts/drive_publish.py` 사용. 루트=`DRIVE_OUTPUTS_ROOT`, 기간=`DRIVE_PUBLISH_PERIOD`. 상세: `docs/guide/drive-archive.md`.
