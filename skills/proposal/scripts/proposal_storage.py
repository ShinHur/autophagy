...
class ProposalPaths:
    ...
    def from_environment(cls) -> ProposalPaths:
        ...
        return cls(
            workspace_root=Path(os.environ.get("PROPOSAL_WORKSPACE_ROOT", str(config_env.private_root() / "proposals"))).expanduser(),
            status_root=Path(
                os.environ.get("PROPOSAL_STATUS_ROOT", str(config_env.private_root() / "proposal-status"))
            ).expanduser(),
            rules_file=Path(
