...
up to 3 candidates → peer slot approval (§2) → owner approval through
the W3-1 gated calendar skill → gws insert → a terse confirmation notice.
§2 coordination envelopes flow through the configured interop route.
Deadlock (no response within the timeout, or 0 candidates) sends an
escalation notice and terminates with ZERO calendar writes. A refusal
...
def build_parser() -> argparse.ArgumentParser:
    ...
    finalize = sub.add_parser("finalize", help="소유자의 `실행 <draft-id>` 답장 후 실행+통지")
