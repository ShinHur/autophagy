...
4. `--review`는 문서 본문 없이 파일명·SHA256·type/version만 소유자(owner)에게 보낸다. 제출·메일·외부 수집 기능은 없다.
...
대화 중 소유자가 문서를 첨부하거나 지목하면 gateway가 제공한 내려받은 첨부 경로만 사용한다. 본문을
...
소유자가 준 사실을 사설 JSON 파일에만 담고 그 경로를 `--inputs-json`에 준다. `--out`도 사설 경로여야 하며,
...
3. 보고서·공고문·안내문·계획서 등 파일 산출물을 생성·저장하라고 하면서 목적지를 말하지 않으면 소유자 본인 Google Drive를 기본 목적지로 사용한다. 공개·링크 공유 권한 없이 비공개로 업로드하고, 재다운로드 SHA-256과 owner-only permissions를 확인한다.
