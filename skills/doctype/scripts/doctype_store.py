...
class StorePaths:
    ...
    def from_environment(cls) -> StorePaths:
        ...
        return cls(
            ...
            overlay_root=Path(
                os.environ.get("DOCTYPE_OVERLAY_ROOT", str(config_env.private_root() / "doctype-library" / "entries"))
            ).expanduser(),
            private_root=Path(
                os.environ.get("DOCTYPE_PRIVATE_ROOT", str(config_env.private_root() / "doctype" / "private"))
            ).expanduser(),
            rules_file=Path(
