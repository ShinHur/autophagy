...
if (REPO_ROOT / "skills" / "doctype" / "scripts").is_dir():
    ...
    # `skills` and `skills.doctype` with __path__ at the skill root. Python then
    # resolves every subpackage (scripts, vendor, ...) itself, keeping the
    # absolute `from skills.doctype...` imports (used by submodules too) working
...
def _review(result: doctype_generate.DraftResult, entry: doctype_store.StoredDocType) -> str:
    ...
    message = (
        ...
        "검토·승인은 소유자가 직접 수행합니다. 이 스킬은 제출하지 않습니다."
    )
