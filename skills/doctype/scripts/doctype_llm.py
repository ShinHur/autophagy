...
def _log_path() -> Path:
    configured = os.environ.get("DOCTYPE_LLM_LOG")
    if configured:
        return Path(configured).expanduser()
    from automation import config_env

    return config_env.private_root() / "doctype" / "logs" / "llm-calls.jsonl"
...
def _litellm_key() -> str:
    ...
    try:
        lines = (config_env.private_root() / ".env.secrets").read_text(encoding="utf-8").splitlines()
    except OSError:
        lines = []
    for line in lines:
