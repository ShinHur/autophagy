---
name: recall
description: "개인 RAG(personal-memory) 검색 스킬. `!recall <질문>` 명시 호출 + 개인/프로젝트 지식 질문에 자동 사용. 검색 결과를 출처(위키 경로/회의/보고 id)와 함께 인용하고, 결과가 없으면 반드시 '기억 없음'이라고 답한다(지어내기 금지). RAG 노드 다운 시 '검색 불가' 안내 후 일반 답변. W2-5."
version: 1.1.0
author: anonymous
license: MIT
platforms: [linux]
...
소유자의 개인 RAG(`personal-memory` 컬렉션, W2-1 MCP memory server)를 검색해
...
2. **자동 사용**: 소유자의 개인/프로젝트/팀 지식(위키에 정리한 내용, 회의,
...
1. **status=hit** → `results[*].excerpt`를 근거로 답하고, 답변 끝에 반드시
   ...
   출처 문자열은 `results[*].attribution`을 그대로 쓴다(위키 경로/회의 문서/보고 채널 메시지 id/task id/세션 id — W2-4가 붙인 메타데이터).
...
4. 검색 결과의 민감한 원문(위키 노트 전문 등)은 소유자 DM 밖(공개 채널,
