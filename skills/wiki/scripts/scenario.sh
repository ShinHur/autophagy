...
cd "$work"  # deploy runs from an inaccessible sandbox CWD; GNU find exits 1 restoring CWD otherwise
...
out="$(cli draft --title "게이트 테스트 노트" --tags "test,w2-2" \
  --body "확인 게이트 검증용 본문." --channel-id 1000000000000001)"
...
set +e
bad_out="$(cli draft --title "" --tags "ok" --body "x" --channel-id 1000000000000001 2>&1)"
bad_rc=$?
set -e
...
wiki_gate._write_json(
    wiki_gate._draft_path(draft["id"]),
    {
        **draft,
        "kind": "wiki",
        "surface": "owner-dm",
        "channel_id": os.environ["WIKI_APPROVALS_CHANNEL_ID"],
        "policy_version": 1,
    },
)
PY
  ...
  export WIKI_APPROVALS_CHANNEL_ID="1000000000000001"
  persist_e2e_binding "$draft_id"

  cli sign --draft "$draft_id" --out "$work/forged.json" \
    --user-id "$test_owner" --channel-id 1000000000000001 --forge-signature >/dev/null
  ...
  cli sign --draft "$draft_id" --out "$work/ok.json" \
    --user-id "$test_owner" --channel-id 1000000000000001 >/dev/null
...
MD
  ...
  twin_out="$(cli draft --title "트윈 결정 노트" --slug twin-decision-note \
    --tags "twin,decision" --channel-id 1000000000000001 \
  ...
  # Canonical synthetic ID; this principle draft needs a distinct channel fixture.
  princ_out="$(cli draft --title "트윈 원칙 노트" --slug twin-principle-note \
    --tags "twin,principle" --channel-id 100000000000000006 \
    --kind principle --authority default --provenance stated \
    --body "$(printf '## Trigger\n호의를 베푼 상대와 좋은 관계일 때.\n\n## Rule\n합리적 범위에서 배려한다.\n\n## Exceptions\n기관 규정·공정성·보안·소유자 이해관계 우선.')")"
  ...
  twin2_out="$(cli draft --title "트윈 취소 노트" --slug twin-cancel-note \
    --tags "twin" --channel-id 1000000000000001 \
...
links: [beta-note]
---
alpha body mentions alpha-term
MD
...
beta body
MD
cli query alpha-term | grep -q 'slug=alpha-note' || fail "query round-trip missed alpha-note"
...
twin_bad="$(cli draft --title "트윈 배드 에넘" --slug twin-bad-enum --tags "twin" \
  --channel-id 1000000000000001 --kind decision --authority binding --provenance stated \
  --body "x" 2>&1)"
twin_bad_rc=$?
set -e
