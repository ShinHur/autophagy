...
# where $script_dir/../../.. may not carry the automation package. Resolve
# AUTOPHAGY_REPO_ROOT to a checkout that actually carries automation/ (and
# entity_preflight), preferring an explicit override, then the script-relative repo.
if [[ -z "${AUTOPHAGY_REPO_ROOT:-}" ]]; then
  AUTOPHAGY_REPO_ROOT="$(cd "$script_dir/../../.." && pwd)"
fi
