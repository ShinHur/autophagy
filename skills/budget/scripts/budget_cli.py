...
Env: BUDGET_DB (~/state/budget.db), BUDGET_GATE_DIR (~/.hermes/budget-gate),
     ...
     BUDGET_SHEET_ID, BUDGET_SHEET_FILE (fixture), AUTOPHAGY_REPO_ROOT,
...
class BudgetConfigError(RuntimeError):
    """A required budget environment variable is absent."""

    variable: str

    def __str__(self) -> str:
        return f"필수 환경변수 {self.variable}가 비어 있습니다 (.env.example에서 설정하세요)"
...
def _sheet_id() -> str:
    ...
    if not sheet_id:
        raise BudgetConfigError(variable="BUDGET_SHEET_ID")
    return sheet_id


def read_balance_values() -> list[list[str]]:
    ...
    result = subprocess.run(  # noqa: S603 — frozen read-only argv
        [budget_gate.gws_bin(), "sheets", "+read", "--spreadsheet", sheet_id,
...
def cmd_query(args: argparse.Namespace) -> int:
    ...
    print(f"BUDGET-OK n={len(selected)} sheet_sha={budget_core.snapshot_hash(rows)[:12]}")
    return 0
...
def _snapshot(*, post: bool) -> int:
    ...
    last = budget_store.latest_snapshot(db)
    if last is None:
        budget_store.store_snapshot(db, new_hash, rows, budget_core.utc_now())
        print(f"BASELINE stored hash={new_hash[:12]}")
        return 0
    prev_hash, prev_rows = last
    if prev_hash == new_hash:
        print(f"NO-CHANGE hash={new_hash[:12]}")
        return 0
    changes = budget_core.diff_rows(prev_rows, rows)
    change_key = budget_core.claim_key(prev_hash, new_hash)
...
def cmd_snapshot(args: argparse.Namespace) -> int:
    try:
        return _snapshot(post=not args.no_post)
    except (SheetAccessError, budget_core.SheetSchemaError) as error:
        reason = budget_core.redact(str(error))[:300]
        retry_id = budget_store.queue_retry(_db_path(), reason, budget_core.utc_now())
...
def cmd_watch(args: argparse.Namespace) -> int:
    ...
    for draft in budget_gate.list_drafts():
        ...
        if action == budget_confirm.CANCEL_EMOJI:
            ...
            budget_confirm.dm_owner("과제비 요청 메일 발송 취소됨")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("status", help="예산 원장 상태 조회").set_defaults(func=cmd_query, item="")
