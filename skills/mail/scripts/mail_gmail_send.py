...
def _build_new_mail_action(request: NewMailRequest) -> CanonicalMailAction:
    ...
    match account:
        ...
        case "site":
            argv = _site_send_argv(request, attachments)
        case unreachable:
            assert_never(unreachable)
...
def _build_reply_mail_action(request: ReplyMailRequest) -> CanonicalMailAction:
    ...
    match account:
        case "gmail":
            argv = _gmail_reply_argv(
                request, _reply_message_id(request.reply_message_id), attachments
            )
        case "site":
            argv = _site_send_argv(request, attachments)
        case unreachable:
...
def _site_send_argv(
    ...
    request_attachments = tuple(
        site_mail_backend.Attachment(
            source_path=entry.source_path_private,
    ...
    backend_request = site_mail_backend.SendRequest(
        to=request.to,
    ...
    return site_mail_backend.build_send_argv(
        backend_request, site_mail_backend.config_sha256()
    )
