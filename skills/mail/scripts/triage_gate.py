"""Draft store + approval records + site-mail send execution for W4-2 (제약 1/6).
...
(hash parity with the deployed pre_tool_call gate) plus a W0-6 audit record to
approvals.jsonl, then executes the exact bridge send argv
...
def _approval_log() -> Path:
    override = os.environ.get("TRIAGE_APPROVAL_LOG")
    if override:
        return Path(override).expanduser()
    config_env = site_mail_backend._config_env()
    try:
        return config_env.private_root() / "logs" / "approvals.jsonl"
    except config_env.ConfigError as error:
        raise GateError("TRIAGE_APPROVAL_LOG 또는 private root 설정이 필요합니다", 3) from error
...
def create_draft(
    *, uid: str, sender: str, mail_subject: str, to: str, subject: str, body: str,
    ...
    backend_hash = site_mail_backend.config_sha256()
    backend_attachments = tuple(
        site_mail_backend.Attachment(
            source_path=str(item["source_path_private"]),
    ...
    argv = site_mail_backend.build_send_argv(
        site_mail_backend.SendRequest(
            ...
            attachment_manifest_sha256=(
                triage_core.attachment_manifest_sha256(attachments) if attachments else None
            ),
        ),
        backend_hash,
    )
...
def _send_payload(stdout: str) -> dict:
    """Parse the bridge JSON contract even when the subprocess exits non-zero."""
    try:
...
def execute_draft(draft: dict, approval: Approval) -> None:
    ...
    backend_hash = draft.get("site_backend_config_sha256")
    if not isinstance(backend_hash, str) or not backend_hash:
        raise GateError("site backend config hash 누락 — 실행 중단", 3)
    site_mail_backend.verify_config_sha256(backend_hash)
        ...
        if error["error_code"] not in _NON_SEND_FAILURE_CODES:
            ...
            if failures >= FAILURE_DOWNGRADE_THRESHOLD:
                triage_mode.downgrade_to_no_go(
                    f"approved site mail send failed {failures} consecutive times (rc={rc})"
                )
        else:
