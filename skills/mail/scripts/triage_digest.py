...
def build_item(
    ...
    A summarize failure keeps the item with a fallback summary, and a classify
    failure (slow/unparseable glm-5.2 output) falls open to a conservative
    중요 verdict with NO actionable flags plus a ``classification_failed`` marker
    (fail-open listing — one bad mail must not silently strand the whole digest).
    """
    ...
    except (triage_llm.LlmCallError, triage_core.LlmParseError):
        # fail-open: one bad classify must never abort the digest (mirrors summarize)
        cls, classify_failed = _CLASSIFY_FALLBACK, True
    try:
        summary = triage_llm.summarize(  # constraint-6 routing lives inside summarize
            subject=subject, sender=sender, body=body, sensitive=gate.sensitive,
            uid_opaque=uid_opaque,
            prompt_path=triage_transport._env_path(
...
def run_digest(*, limit: int, sync: bool, dry_run: bool) -> int:
    ...
    with a warning that the configured site backend may need attention.
    ...
    if sync_failed:
        body = "⚠️ site backend 동기화 실패 — 마지막 조회 결과 기준\n" + body
    if dry_run:
        print(body)
