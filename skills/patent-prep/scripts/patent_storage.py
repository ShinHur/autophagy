...
class PatentPaths:
    ...
    def from_environment(cls) -> PatentPaths:
        ...
        return cls(
            Path(os.environ.get("PATENT_DRAFT_ROOT", str(config_env.private_root() / "patent-drafts"))).expanduser(),
            Path(os.environ.get("PATENT_STATUS_ROOT", str(config_env.private_root() / "patent-status"))).expanduser(),
        )
