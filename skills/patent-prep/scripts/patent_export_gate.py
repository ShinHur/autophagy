...
def _interop_config() -> Path:
    return Path(os.environ.get("INTEROP_CONFIG", str(config_env.private_root() / "interop" / "config.json"))).expanduser()


def _env_secrets() -> Path:
    return config_env.private_root() / ".env.secrets"


def owner_id() -> str:
    config = _interop_config()
    try:
...
def bot_token() -> str:
    ...
    if token:
        return token
    try:
        lines = _env_secrets().read_text(encoding="utf-8").splitlines()
    except OSError:
        lines = []
    for line in lines:
