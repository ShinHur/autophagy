---
name: procurement
description: "이미 등록된 구매/용역 템플릿에 품목·금액·업체를 채워 초안 파일을 생성하고 소유자에게만 검토 요청을 보낸다. 새 문서의 등록·스킬화·문서 종류 저장·개선은 doctype 소유이며, 이 스킬은 새로 받은 문서를 템플릿으로 저장하지 않는다."
version: 1.2.0
...
이미 등록된 소유자의 기관 양식에 값을 채워 **초안 파일**을 만들고 검토를 요청한다.
...
1. **제출은 항상 인간**: 이 스킬은 어떤 문서도 외부(기관 시스템/메일/타인)로
   제출·발송하지 않는다. 유일한 발신은 소유자 본인에게 보내는 검토 요청 DM이다.
2. **바이너리 .hwp로는 생성 시도 금지**: preflight가 `CONVERSION-REQUEST`(exit 3)를
   내면 그 안내문을 그대로 소유자에게 전달하고 **생성을 시도하지 않는다**
   (한글에서 HWPX로 변환한 사본을 요청). 지원 포맷: docx/xlsx/hwpx.
3. **필수 항목 누락 시 생성 거부**: `GENERATION-REFUSED`(exit 5)가 나열한 누락
   항목을 소유자에게 질문해 채운 뒤에만 다시 시도한다. 값을 추측하지 마라.
4. **금액/업체명은 공개 채널·repo에 게시 금지**: 수집한 값과 생성 파일은 소유자 DM과
...
`templates-show --name <등록된-이름>`의 fields 순서대로 소유자에게 물어보고, 받은 값을
...
재확인한다. `GENERATION-REFUSED`(exit 5)면 누락 목록을 소유자에게 안내한다.
...
25MiB 이하는 파일 첨부, 초과는 소유자 본인 Drive의 `Example Organization/procurement/<YYYY-MM>/` 폴더에 업로드 후 링크를 DM한다(최상위 루트 아님). 루트명은 `PROCURE_DRIVE_ROOT`로 조정 가능.
DM 말미의 "제출은 소유자가 직접" 문구를 지우지 마라.
