...
The ONLY outbound surface of this skill: a DM to the owner asking for
human review. ≤25 MiB attaches the draft; larger files are uploaded to the owner's
...
USER_AGENT = "DiscordBot (https://example.invalid/autophagy, 0)"
...
def owner_id() -> str:
    config_path = os.environ.get("INTEROP_CONFIG", "")
    if not config_path:
        from automation import config_env

        config_path = str(config_env.private_root() / "interop" / "config.json")
...
def review_note(file: Path, mode: str, note: str, link: str) -> str:
    ...
    return body + "\n검토 후 **제출은 소유자가 직접** 해주세요 — 이 스킬은 어디에도 제출하지 않습니다."
...
def _default_drive_root() -> str:
    ...
    return f"{config_env.organization_label()} 산출물"


def _drive_root_name() -> str:
    return os.environ.get("PROCURE_DRIVE_ROOT", _default_drive_root())
