...
Subcommands mirror the DM conversation the agent drives with the owner:
...
if (REPO_ROOT / "skills" / "procurement" / "scripts").is_dir():
    ...
    # Deployed layout has no importable `skills` package above it, so synthesize
    # `skills` and `skills.procurement` with __path__ at the skill root. Python
    # then resolves every subpackage (scripts, vendor, ...) itself, keeping the
    # absolute `from skills.procurement...` imports (used by submodules too)
    # working from the immutable store.
    import types
...
def _audit(event: str, detail: str) -> None:
    ...
    log = Path(os.environ.get("PROCURE_AUDIT_LOG", str(config_env.private_root() / "procurement" / "audit.log"))).expanduser()
